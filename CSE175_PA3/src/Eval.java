...
public class Eval {
    ...
    static public int depth_limit = 2; //TA recommended to set the depth limit to 2, in order to speed up the value_roll_hand method
    ...
    {
    	double val = 0.0;
    	//All code was discussed about in Lab by the TA
    	
    	for(DieFace d1 : DieFace.values())//Sort through all possibilities of the first Die
    	{
    		if(d1 != DieFace.invalid)//Should not have a Die with value "invalid"
    		{
    			for(DieFace d2 : DieFace.values())//Sort through all possibilities of the second Die
    			{
    				if(d2 != DieFace.invalid)//Should not have a Die with value "invalid"
    				{
    					for(DieFace d3 : DieFace.values())//Sort through all possibilities of the third Die, now all possibilities are accounted for 
    					{
    ...
    {
    	//This heuristic makes a cautious computer player, in which it very carefully avoids being shotgunned, but it will occasionally take a risk based on the feet received
    	double value = 0.0;
    	if(s.blasts_collected == 0 || s.brains_collected == 0)
    		return(-s.win_payoff);//If the computer has not collected any brains or it has not collected any blasts, there is no point to not roll again
    	
    	double feet = 0;
    	for (Die d : s.hand)//Cycle through the dies in the player's hand
    	{
    			
    			if (d.color == DieColor.red)//Rolling red feet is not good
    			{
    				feet = -d.Pfeet;
    			}
    			else if(d.color == DieColor.yellow)//Rolling yellow feet is risky
    			{
    				feet = d.Pfeet/2;
    			}
    			else if(d.color == DieColor.green)//Rolling green feet is worthwhile
    			{
    				feet = d.Pfeet;
    			}
    			else
    				feet = 0;//Should not reach here
